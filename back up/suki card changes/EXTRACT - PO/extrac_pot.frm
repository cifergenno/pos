...
MsgBox "Genarate PO from sales inventory starting from " & Combo1.Text & " " & Combo2.Text & ", " & Combo3.Text & " to " & Combo4.Text & " " & Combo5.Text & ", " & Combo6.Text
Dim texto As String
...
conn.Execute "INSERT INTO po_extrac (ITEM_CODE)" _
& "values ('" & fld.Value & "')"
...
adlaw = Abs(adlaw) + Abs((buwan * 30))


age_err2:

conn.Execute "UPDATE po_extrac SET AGEING = " & "'" & adlaw & "'" & " WHERE ITEM_CODE = '" & item_ccode & "'"
adlaw = 0
buwan = 0
tuig = 0
Next
...
On Error GoTo nextnext
    rs18.Open "SELECT QTY_SOLD FROM po_extrac WHERE ITEM_CODE = '" & item_ccode & "'", conn
...
Do Until rs18.EOF
    ...
    For Each fld1 In rs18.Fields
        ...
        If fld1.Value = "" Then
        conn.Execute "UPDATE po_extrac SET QTY_SOLD = " & "'" & fld.Value & "'" & " WHERE ITEM_CODE = '" & item_ccode & "'"
        Else
        conn.Execute "UPDATE po_extrac SET QTY_SOLD  = " & "'" & Val(fld.Value) + Val(fld1.Value) & "'" & " WHERE ITEM_CODE = '" & item_ccode & "'"
        End If
        rs18.MoveNext
    Next
   'rs18.MoveNext
...
'MsgBox mm.Text & dd.Text & yy.Text
...
Set ObjWs = ObjWb.Worksheets.Add
    ...
    ObjWs.Cells(1, 11) = "ORDER"
     ...
     rs20.Open "SELECT * FROM PO_extrac", conn
...
Loop
     ...
     'Me.Visible = True
    ObjWb.SaveAs ("d:\PO report on sales inventory starting from " & texto & " to " & texto1 & ".xls")
