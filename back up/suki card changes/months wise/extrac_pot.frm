...
For aaaa = 0 To Val(Text4.Text)
    ...
    List2.AddItem report(1)
        ...
        rs5.Open "SELECT S_D FROM date_wise where DATE_SOLD like '" & report(1) & "%'", conn
...
mail:
               ...
               If report(9) <> 0 Then
                conn.Execute "INSERT INTO date_wise (DATE_SOLD,GROSS_SALE, GROSS_MARGIN,S_D,QTY_SOLD) values ('" & report(1) & "','" & report(11) * Val(report(6)) & "','" & (Val(report(10)) - Val(report(9))) * Val(report(6)) & "','" & Val(report(12)) + Val(report(13)) & "','" & report(6) & "')"
...
If ok = False Then
        ...
        Else: report(13) = "0"
        End If
        List5.AddItem report(12)
        
         
        For Each fld2 In rs5.Fields
        conn.Execute "UPDATE date_wise SET S_D = '" & Val(report(12)) + Val(report(13)) + Val(fld2.Value) & " ' WHERE DATE_SOLD = '" & report(1) & "'"
        Next
...
Set ObjWs = ObjWb.Worksheets.Add
    ...
    Loop
     ...
     'Me.Visible = True
    ObjWb.SaveAs ("d:\DATE WISE report starting from " & texto & " to " & texto1 & ".xls")
