...
MsgBox "Genarate Category Wise report starting from " & Combo1.Text & " " & Combo2.Text & ", " & Combo3.Text & " to " & Combo4.Text & " " & Combo5.Text & ", " & Combo6.Text
Dim texto As String
...
For aaaa = 0 To Val(Text4.Text)
    ...
    Next
List2.AddItem report(2)

        rs2.Open "SELECT GROSS_SALE FROM cat_wise where CATEGORY = '" & report(2) & "'", conn
        rs3.Open "SELECT GROSS_MARGIN FROM cat_wise where CATEGORY = '" & report(2) & "'", conn
        rs4.Open "SELECT QTY_SOLD FROM cat_wise where CATEGORY = '" & report(2) & "'", conn
...
mail:
               ...
               If report(9) <> 0 Then
                ...
                conn.Execute "INSERT INTO cat_wise (CATEGORY,GROSS_SALE, GROSS_MARGIN,QTY_SOLD) values ('" & report(2) & "','" & report(11) * Val(report(6)) & "','" & ((Val(report(10)) - Val(report(9))) * Val(report(6))) - Val(report(12)) & "','" & report(6) & "')"
                List4.AddItem Val(report(10)) - Val(report(9))
...
If ok = False Then
        ...
        conn.Execute "UPDATE cat_wise SET GROSS_SALE = '" & (Val(report(11)) * Val(report(6))) + Val(fld2.Value) & " ' WHERE CATEGORY = '" & report(2) & "'"
        Next
        
        For Each fld2 In rs3.Fields
        List4.AddItem Val(report(10)) - Val(report(9))
        conn.Execute "UPDATE cat_wise SET GROSS_MARGIN = '" & (((Val(report(10)) - Val(report(9))) * Val(report(6))) + Val(fld2.Value)) - Val(report(12)) & "' WHERE CATEGORY = '" & report(2) & "'"
        
        Next
        
        For Each fld2 In rs4.Fields
        conn.Execute "UPDATE cat_wise SET QTY_SOLD = '" & Val(report(6)) + Val(fld2.Value) & "' WHERE CATEGORY = '" & report(2) & "'"
        Next
...
Set ObjWs = ObjWb.Worksheets.Add
    ...
    Loop
     ...
     'Me.Visible = True
    ObjWb.SaveAs ("d:\CATEGORY WISE report starting from " & texto & " to " & texto1 & ".xls")
